...
public class ReservaController {
    ...
    public Resultado crearReserva(@RequestBody CrearReservaRequestDTO request) {
        ...
        try {
            reservaService.realizarReserva(request); // Llama al flujo del diagrama

            resultado.id = 0;
            resultado.mensaje = "Reserva realizada con éxito"; // Mensaje final del diagrama
        } catch (IllegalArgumentException e) {
            // Errores de validación (Fechas, datos faltantes)
            resultado.id = 2;
            resultado.mensaje = "Error de validación: " + e.getMessage();
        } catch (Exception e) {
            // Error interno
            resultado.id = 1;
